...
When set to false, a new userns is created for the pod. +
This field is alpha-level and is only honored by servers that enable the UserNamespacesSupport feature. + |  | 
| *`externalDatastore`* __xref:{anchor_prefix}-github-com-rancher-k3k-pkg-apis-k3k-io-v1beta1-externaldatastoreconfig[$$ExternalDatastoreConfig$$]__ | ExternalDatastore sets the configuration for external database via kine. +
...
[id="{anchor_prefix}-github-com-rancher-k3k-pkg-apis-k3k-io-v1beta1-externaldatastoreconfig"]
...
|===
| Field | Description | Default | Validation
| *`datastoreEndpoint`* __string__ | DatastoreEndpoint defines the connection string for external datastore + |  | 
| *`datastoreCertSecret`* __link:https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#secretvolumesource-v1-core[$$SecretVolumeSource$$]__ | DatastoreCertSecret defines the external datastore certificate and key pair + |  | 
| *`datastoreCACertSecret`* __link:https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.31/#secretvolumesource-v1-core[$$SecretVolumeSource$$]__ | DatastoreCACertSecret defines the external datastore CA certificate and key pair + |  | 
|===
